...
foreach ($groupKey in $groups) {
    ...
    if ($Mode -eq "Apply") {
        ...
        if (-not $existingWorkItem -or $null -eq $existingWorkItem.qfu_status) { $body.qfu_status = $option.openWorkItemStatus }
...
$result = [ordered]@{
    ...
    applyModeHardening        = [ordered]@{
        ...
        statusPreservation            = "Existing qfu_status is only set when blank/null; new work items are set to Open."
