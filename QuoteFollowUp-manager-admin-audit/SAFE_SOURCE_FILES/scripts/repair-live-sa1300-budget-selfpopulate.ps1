...
function Get-Sa1300DefinitionState {
  ...
  return [pscustomobject]@{
    ...
    active_budget_select = if (
      $rootActions.PSObject.Properties["Guard_Budget_Row_Limit"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.PSObject.Properties["Get_Active_Budget"]
    ) {
      [string]$rootActions.Guard_Budget_Row_Limit.actions.Get_Active_Budget.inputs.parameters.'$select'
    } else {
      $null
    }
    active_budget_select_ok = [bool](
      $rootActions.PSObject.Properties["Guard_Budget_Row_Limit"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.PSObject.Properties["Get_Active_Budget"] -and
      [string]$rootActions.Guard_Budget_Row_Limit.actions.Get_Active_Budget.inputs.parameters.'$select' -eq $expectedActiveBudgetSelect
    )
    same_month_actual_expression = if (
      $rootActions.PSObject.Properties["Guard_Budget_Row_Limit"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.PSObject.Properties["Condition_Check_Month_Changed"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.Condition_Check_Month_Changed.else.actions.PSObject.Properties["Condition_Budget_Exists_Same_Month"]
    ) {
      [string]$rootActions.Guard_Budget_Row_Limit.actions.Condition_Check_Month_Changed.else.actions.Condition_Budget_Exists_Same_Month.actions.Update_Current_Month_Budget.inputs.parameters.item.qfu_actualsales
    } else {
      $null
    }
    same_month_actual_expression_ok = [bool](
      $rootActions.PSObject.Properties["Guard_Budget_Row_Limit"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.PSObject.Properties["Condition_Check_Month_Changed"] -and
      $rootActions.Guard_Budget_Row_Limit.actions.Condition_Check_Month_Changed.else.actions.PSObject.Properties["Condition_Budget_Exists_Same_Month"] -and
      [string]$rootActions.Guard_Budget_Row_Limit.actions.Condition_Check_Month_Changed.else.actions.Condition_Budget_Exists_Same_Month.actions.Update_Current_Month_Budget.inputs.parameters.item.qfu_actualsales -eq $expectedPreservedActualSalesExpression
    )
    delete_ops_daily_foreach_expression_ok = [bool](
      $rootActions.PSObject.Properties["Delete_Existing_Branch_Ops_Daily"] -and
      [string]$rootActions.Delete_Existing_Branch_Ops_Daily.foreach -eq $expectedDeleteOpsDailyForeachExpression
    )
    analytics_cad_ops_daily_expression_ok = [bool](
      $rootActions.PSObject.Properties["Condition_Current_Month_Budget_Record_For_Analytics_Exists"] -and
      [string]$rootActions.Condition_Current_Month_Budget_Record_For_Analytics_Exists.actions.Update_Current_Month_Budget_Analytics_Payload.inputs.parameters.item.qfu_opsdailycadjson -eq $expectedPreservedCadOpsDailyExpression
    )
    analytics_usd_ops_daily_expression_ok = [bool](
      $rootActions.PSObject.Properties["Condition_Current_Month_Budget_Record_For_Analytics_Exists"] -and
      [string]$rootActions.Condition_Current_Month_Budget_Record_For_Analytics_Exists.actions.Update_Current_Month_Budget_Analytics_Payload.inputs.parameters.item.qfu_opsdailyusdjson -eq $expectedPreservedUsdOpsDailyExpression
    )
  }
}
...
function Repair-LiveBudgetDefinition {
  ...
  foreach ($property in @($liveBudgetRoot.PSObject.Properties)) {
    ...
    if ($property.Name -in $opsDailyActionNames) {
      $canonicalOpsDailyProperty = $canonicalBudgetRoot.PSObject.Properties[$property.Name]
      if ($canonicalOpsDailyProperty) {
        $canonicalOpsDailyAction = $canonicalOpsDailyProperty.Value | ConvertTo-Json -Depth 100 | ConvertFrom-Json
        Apply-HostConnectionMap -Node $canonicalOpsDailyAction -HostConnectionMap $liveHostConnectionMap
        $updatedBudgetRootActions[$property.Name] = $canonicalOpsDailyAction
        continue
      }
    }
...
Write-Utf8Json -Path $OutputPath -Object $report
  ...
  Select-Object branch_code, patched,
    ...
    @{ Name = "active_select_ok"; Expression = { if ($_.after) { $_.after.active_budget_select_ok } else { $null } } },
    @{ Name = "actual_guard_ok"; Expression = { if ($_.after) { $_.after.same_month_actual_expression_ok } else { $null } } },
    @{ Name = "ops_delete_ok"; Expression = { if ($_.after) { $_.after.delete_ops_daily_foreach_expression_ok } else { $null } } },
    @{ Name = "ops_cad_ok"; Expression = { if ($_.after) { $_.after.analytics_cad_ops_daily_expression_ok } else { $null } } },
    @{ Name = "ops_usd_ok"; Expression = { if ($_.after) { $_.after.analytics_usd_ops_daily_expression_ok } else { $null } } },
